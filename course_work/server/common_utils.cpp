...
bool validateUsername(const std::string& username, std::string& errorMessage) {
    ...
    // Проверка допустимых символов: буквы, цифры, точка, дефис, нижнее подчеркивание
    for (char c : username) {
        if (!std::isalnum(c) && c != '.' && c != '-' && c != '_') {
            errorMessage = "Логин может содержать только буквы, цифры, точку (.), дефис (-) и нижнее подчеркивание (_)";
            return false;
        }
    }
